...
module.exports.updateSkill = function(mySkillID, inputRequestJson, accountAccesToken){
     ...
     console.log(myInteractionModel);

     var options = {
		  method: 'POST',
	      uri: config.amazonConfig.amazonSkillBaseUrl + mySkillID + "/interactionModel/locales/en-US",
...
function buildInteractionModel(input){
    ...
    interactionModelBuilder.interactionModel.languageModel.intents = [];
	   ...
	   if (input.intentsList.hasOwnProperty(key)) {
	      ...
	      if(slotsArray.length>0){
	          ...
	          for(var i=0; i<slotsArray.length; i++){
	            ...
	            slotObj.type = "AMAZON.Movie";

	            // if(slotObj.name == "category"){
	            //   slotObj.type = "AMAZON.Movie";
	            // }else{
	            //   slotObj.type = "AMAZON.MovieSeries";
	            // }
	          }
